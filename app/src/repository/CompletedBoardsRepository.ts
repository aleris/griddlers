...
export class CompletedBoardsRepository {
  ...
  static async getBoard(packId: string, boardId: string): Promise<PersistedBoard> {
    const item = (await this.getStore(packId).getItem(boardId)) as PersistedBoard | null;
    if (item === null) {
  ...
  static async listBoardsForPack(packId: string): Promise<PersistedBoard[]> {
    ...
    return Promise.all(
      ids.map(async (id): Promise<PersistedBoard> => await this.getBoard(packId, id))
    );
  }
